...
const ChatBot: React.FC = () => {
  ...
  useEffect(() => {
    const initializeSocket = () => {
      socket.current = io("https://bookcelebrity-server.onrender.com");
      socket.current.on(
        "receiveMessage",
  ...
  const fetchChatHistory = (sessionId: string) => {
    axios
      .get(
        `https://bookcelebrity-server.onrender.com/api/chat/history/${sessionId}`
      )
      .then((response) => {
        setMessages(
  ...
  const handleSendMessage = () => {
    if (userInput.trim()) {
      if (!sessionId) {
        if (userName) {
          axios
            .post("https://bookcelebrity-server.onrender.com/api/chat/start", {
              chatUser: userName,
            })
            .then((response) => {
